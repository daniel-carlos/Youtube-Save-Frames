...
start_time = 10  # posição (em segundos) da primeira captura
# limite de parada (em segundos). 0 ou valores negativos, limite passa a ser a duração do próprio vídeo
end_time = 0
...
def save_frame(cap, filename):
    ...
    if (len(results) > 0):
        ...
        occurences = []
        for cl in cls:
            occurences.append(names[cl])

        has_obj = False
        for targetName in targets:
            if targetName in occurences:
                has_obj = True

    if has_obj:
        save_image(frame, filename)
    else:
        print(f"Skipping {filename} because has not occurences")
    return True
...
def capture_frames(video_url, interval=5, start_time=0, end_time=-1):
    ...
    end_frame = int(min(end_time, duration_s) *
                    fps) if end_time > 0 else (duration_s * fps)
